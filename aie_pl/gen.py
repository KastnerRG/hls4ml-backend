...
def host_cpp(batch, n_in, n_out, aie_groups_info):
    in_bytes  = batch * n_in
...
def main():
    ...
    aie_groups_info = [(gi, a.n_iter * aie_groups[gi][-1][2] * a.batch)
                       for gi, _ in aie_segs]
    write_if_changed("sw/data.h",    sw_data_h(x0, ref_out))
    write_if_changed("sw/host.cpp",  host_cpp(a.batch, LAYERS[0][1], LAYERS[-1][2], aie_groups_info))
    ...
    # Config for check.py
    pl_cycles = sum(a.batch * LAYERS[idx][1] // a.reuse_factor
