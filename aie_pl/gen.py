...
def host_cpp(batch, n_in, n_out, aie_groups_info, n_iter=1):
    ...
    run_once = (f"        auto s = s2mm_k(out_bo, nullptr, {out_words});\n"
                f"        auto m = mm2s_k(in_bo,  nullptr, {in_words});\n"
                 "        graph.run(1);\n"
                 "        m.wait(); s.wait(); graph.wait();")
...
{wt_updates}
    // Warmup: one un-timed inference to settle pipeline state.
    {{
{run_once}
    }}
    // Timed region: n_iter full pipeline runs, report per-iteration average.
    // n_iter > 1 amplifies the timing signal — useful when quantum is coarse.
...
{run_once}
    ...
    std::cout << "End-to-end (hw_emu): " << total_ns / {n_iter} << " ns\\n";
